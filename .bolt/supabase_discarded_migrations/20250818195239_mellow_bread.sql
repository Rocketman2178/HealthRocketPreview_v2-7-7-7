/*
  ...
  2. Security
    ...
    - Community members can only access their community's messages
    - Users can only delete their own messages
  
  3. Functions
    - Extract mentioned users from message content
    - Handle mention notifications automatically
    - Update reply counts
...
DECLARE
  ...
  matches TEXT[];
BEGIN
  -- Extract all @username patterns from content using regexp_split_to_array
  FOR username_match IN
    SELECT unnest(regexp_split_to_array(p_content, '@(\w+)', 'g'))
    WHERE unnest ~ '^\w+$'
  LOOP
...
BEGIN
  ...
  SELECT 
    ...
    ccm.created_at,
    ccm.updated_at,
    COALESCE(u.name, ccm.user_name, 'Unknown User') as user_name,
